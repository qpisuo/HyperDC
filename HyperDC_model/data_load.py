...
def gen_DGLGraph_simple(ground, gpu=0):
...
def gen_data_simple(dataset_name, gpu=0, dim_edge=400, alpha_e=0, alpha_v=0):
    device = 'cuda:{}'.format(gpu) if gpu != -1 else 'cpu'
    data_path = f'/home/hbyu/HyperDC/data_split/{dataset_name}.pt'
    ...
    if isinstance(node_feat, np.ndarray):
        v_feat = torch.from_numpy(node_feat.astype(np.float32)).to(device)
    else:
        v_feat = torch.from_numpy(np.array(node_feat.astype(np.float32).todense())).to(device)
        
    vidx = NodeEdgePair[:, 0].to(device)
    eidx = NodeEdgePair[:, 1].to(device)
    incidence = torch.zeros(ne, nv)
    for elem in EdgeNodePair:
        e, v = elem
        incidence[e, v] = 1
    v_feat = torch.tensor(node_feat).type('torch.FloatTensor').to(device)
    e_feat = torch.ones(ne, dim_edge).to(device)
    
    v_weight = torch.Tensor([(1/w if w > 0 else 1) for w in nodewt]).unsqueeze(-1).to(device)
    e_weight = torch.Tensor([(1/w if w > 0 else 1) for w in edgewt]).unsqueeze(-1).to(device)
    node2sum = defaultdict(list)
    edge2sum = defaultdict(list)
    e_reg_weight = torch.zeros(ne) 
    v_reg_weight = torch.zeros(nv) 
    for i, (node_idx, edge_idx) in enumerate(NodeEdgePair.tolist()):
        e_wt = e_weight[edge_idx]
        e_reg_wt = e_wt**alpha_e 
        e_reg_weight[edge_idx] = e_reg_wt
        node2sum[node_idx].append(e_reg_wt) 
        
        v_wt = v_weight[node_idx]
        v_reg_wt = v_wt**alpha_v
        v_reg_weight[node_idx] = v_reg_wt
        edge2sum[edge_idx].append(v_reg_wt)      
    ...
    e_reg_sum[e_reg_sum==0] = 1
    v_reg_sum[v_reg_sum==0] = 1
    e_reg_weight = torch.Tensor(e_reg_weight).unsqueeze(-1).to(device)
    v_reg_sum = torch.Tensor(v_reg_sum).unsqueeze(-1).to(device)
    v_reg_weight = torch.Tensor(v_reg_weight).unsqueeze(-1).to(device)
    e_reg_sum = torch.Tensor(e_reg_sum).unsqueeze(-1).to(device)
