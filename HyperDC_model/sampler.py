...
def clique_negative_sampling(
    ...
    while frozenset(neg) in hyperedges or not is_valid_sample(neg):
        edgeidx = np.random.choice(len(hyperedges), size=1)[0]
...
def sized_mf_sampling(size_dist, nodes, nodes_to_neighbors, hyperedges):
    ...
    while frozenset(sampled_nodes) in hyperedges or not is_valid_sample(sampled_nodes):
        sampled_nodes, sampled_edge = mfinder_sampling(nodes_to_neighbors, sampled_size)

    hyperedges.remove(frozenset({'a', 'b'}))
    return sampled_nodes, sampled_edge 

def is_valid_sample(sampled_nodes):
    nodes_list = list(sampled_nodes)
    count_disease_node = sum(1 for node in nodes_list if node >= 2774)
    return count_disease_node == 1
