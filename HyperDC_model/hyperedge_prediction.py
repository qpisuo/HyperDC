...
def train(args):
    # construct the folder for checkpoints and logs
    os.makedirs(f"HyperDC_model/checkpoints/{args.folder_name}", exist_ok=True)
    os.makedirs("HyperDC_model/logs", exist_ok=True) 
    f_log = open(f"HyperDC_model/logs/{args.folder_name}_train.log", "w")
    ...
    for j in tqdm(range(args.exp_num)):    
        ...
        data_dict = torch.load(f'data_split/splits/DCsplit{j}_final.pt')
        ...
        for epoch in tqdm(range(args.epochs), leave=False):            
            ...
            train_label = torch.round(torch.cat(train_label, dim=0)).long()  
            ...
            if best_roc < (auc_roc_sns+auc_roc_mns+auc_roc_cns)/3:
                ...
                best_epoch=epoch
                torch.save(model.state_dict(), f"HyperDC_model/checkpoints/{args.folder_name}/model_{j}.pkt")
                torch.save(Aggregator.state_dict(), f"HyperDC_model/checkpoints/{args.folder_name}/Aggregator_{j}.pkt")
                torch.save(Generator.state_dict(), f"HyperDC_model/checkpoints/{args.folder_name}/Generator_{j}.pkt")
    
    
        with open(f"HyperDC_model/checkpoints/{args.folder_name}/best_epochs.logs", "a") as e_log:  
            e_log.write(f"exp {j} best epochs: {best_epoch}\n")
    f_log.close()
    return args
def test(args, j):    
    args.checkpoint = f"HyperDC_model/checkpoints/{args.folder_name}"
    f_log = open(f"HyperDC_model/logs/{args.folder_name}_results.log", "a")    
    f_log.write(f"experiment {j} \n")    
