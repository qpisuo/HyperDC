...
def model_train(args, g, model, Aggregator, Generator, optim_D, optim_G, pos_hedges, pos_labels, train_pred, train_label, device, epoch):
    ...
    ## Train Generator ##
    if "MLP" in args.gen:
        neg_hedge_onehots, neg_hedge_indices = Generator(batch_size, args.dataset_name)    
...
def model_eval_simple(v_feat, e_feat, v_reg_weight, v_reg_sum, e_reg_weight, e_reg_sum, g, model, Aggregator, node_indices, n_layers):
    model.eval()
    Aggregator.eval()
    with torch.no_grad():
        
        v, e = model([g, v_feat, e_feat, v_reg_weight, v_reg_sum, e_reg_weight, e_reg_sum], n_layers)
        
        
        embeddings = v[node_indices]
...
def model_embedding_UMAP(v_feat, e_feat, v_reg_weight, v_reg_sum, e_reg_weight, e_reg_sum, g, model,  n_layers):
    model.eval()
