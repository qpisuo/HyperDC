# run this script to get the split data
import torch
import numpy as np
...
def split_dataset(data_path):
    ...
    for e in range(ne):
        non_zero_indices = torch.nonzero(incidence[e]).squeeze().tolist()
        HE.append(frozenset(non_zero_indices))
    ...
    for split in range(5):
        ...
        train_mns, train_sns, train_cns = neg_generator(HE, ground_train_num+train_only_num) 
        valid_mns, valid_sns, valid_cns = neg_generator(HE, ground_valid_num+valid_only_num) 
        test_mns, test_sns, test_cns = neg_generator(HE, test_num) 
        ...
        torch.save({'ground_train': ground_train_data, 'ground_valid': ground_valid_data, \
                ...
                f'data_split/splits_HE/DCsplit{split}.pt')
...
if __name__ == '__main__': 
    split_dataset('data_split/hypergraph_pretrain_allKG.pt')
    print('complete')